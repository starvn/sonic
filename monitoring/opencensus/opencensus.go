...
func (c *composableRegister) ExporterFactories(ctx context.Context, cfg Config, fs []ExporterFactory) {
	var viewExporters []view.Exporter
	var traceExporters []trace.Exporter
...
func (c composableRegister) Register(ctx context.Context, cfg Config, vs []*view.View) error {
	...
	if cfg.Exporters.Prometheus != nil {
		for _, viewV := range vs {
			if strings.Contains(viewV.Name, "http/client") {
				if cfg.Exporters.Prometheus.HostTag {
					viewV.TagKeys = appendIfMissing(viewV.TagKeys, ochttp.KeyClientHost)
				}

				if cfg.Exporters.Prometheus.PathTag {
					viewV.TagKeys = appendIfMissing(viewV.TagKeys, ochttp.KeyClientPath)
				}

				if cfg.Exporters.Prometheus.MethodTag {
					viewV.TagKeys = appendIfMissing(viewV.TagKeys, ochttp.KeyClientMethod)
				}

				if cfg.Exporters.Prometheus.StatusCodeTag {
					viewV.TagKeys = appendIfMissing(viewV.TagKeys, ochttp.KeyClientStatus)
				}
			}

			if strings.Contains(viewV.Name, "http/server") {
				if cfg.Exporters.Prometheus.HostTag {
					viewV.TagKeys = appendIfMissing(viewV.TagKeys, ochttp.Host)
				}

				if cfg.Exporters.Prometheus.PathTag {
					viewV.TagKeys = appendIfMissing(viewV.TagKeys, ochttp.Path)
				}

				if cfg.Exporters.Prometheus.MethodTag {
					viewV.TagKeys = appendIfMissing(viewV.TagKeys, ochttp.Method)
				}

				if cfg.Exporters.Prometheus.StatusCodeTag {
					viewV.TagKeys = appendIfMissing(viewV.TagKeys, ochttp.StatusCode)
				}
			}
		}
	}
...
const (
	...
	Namespace  = "github.com/starvn/sonic/monitoring/opencensus"
)
...
var (
	...
	ErrNoConfig                           = errors.New("no extra config defined for the opencensus package")
...
func parseCfg(srvCfg config.ServiceConfig) (*Config, error) {
	...
	_ = json.NewEncoder(buf).Encode(tmp)
