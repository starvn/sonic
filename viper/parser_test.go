...
func testExtraConfig(extraConfig map[string]interface{}, t *testing.T) {
	...
	if parents[0] != "starvn" {
		t.Error("Parent 0 of user us not starvn")
	}
	if parents[1] != "ducdh" {
		t.Error("Parent 1 of user us not ducdh")
	}
	...
	"endpoints": [
			{
					"endpoint": "/github",
					"method": "GET",
					"extra_config" : {"user":"test","hits":6,"parents":["starvn","ducdh"], "nested_data": {"data": {"status": "OK"}}},
					"backend": [
							{
									"host": [
									...
									"extra_config" : {"user":"test","hits":6,"parents":["starvn","ducdh"]}
							}
					]
			},
			{
