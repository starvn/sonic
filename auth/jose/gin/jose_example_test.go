...
import (
	...
	sjose "github.com/starvn/sonic/auth/jose"
...
func Example_RS256() {
	...
	verifierCfg.ExtraConfig[sjose.ValidatorNamespace].(map[string]interface{})["operation_debug"] = true
...
func Example_HS256_cookie() {
	...
	_, signer, _ := sjose.NewSigner(sCfg, nil)
...
func runValidationCycle(signerEndpointCfg, validatorEndpointCfg *config.EndpointConfig) {
	...
	mixedCfg := &config.EndpointConfig{
		...
		ExtraConfig: config.ExtraConfig{
			sjose.SignerNamespace:    signerEndpointCfg.ExtraConfig[sjose.SignerNamespace],
			sjose.ValidatorNamespace: validatorEndpointCfg.ExtraConfig[sjose.ValidatorNamespace],
		},
	}
...
func newSignerEndpointCfg(alg, ID, URL string) *config.EndpointConfig {
	return &config.EndpointConfig{
		...
		ExtraConfig: config.ExtraConfig{
			sjose.SignerNamespace: map[string]interface{}{
				"alg":                  alg,
				"kid":                  ID,
				"jwk_url":              URL,
...
func newVerifierEndpointCfg(alg, URL string, roles []string) *config.EndpointConfig {
	return &config.EndpointConfig{
		...
		ExtraConfig: config.ExtraConfig{
			sjose.ValidatorNamespace: map[string]interface{}{
				"alg":                  alg,
				"jwk_url":              URL,
				"audience":             []string{"http://api.example.com"},
				"issuer":               "http://example.com",
				"roles":                roles,
				"propagate_claims":     [][]string{{"jti", "x-sonic-jti"}, {"sub", "x-sonic-sub"}, {"nonexistent", "x-sonic-ne"}, {"sub", "x-sonic-replace"}},
				"disable_jwk_security": true,
				"cache":                true,
			},
		},
