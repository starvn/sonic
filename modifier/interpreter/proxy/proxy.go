...
func ProxyFactory(l log.Logger, pf proxy.Factory) proxy.Factory {
	return proxy.FactoryFunc(func(remote *config.EndpointConfig) (proxy.Proxy, error) {
		...
		cfg, err := interpreter.Parse(l, remote.ExtraConfig, ProxyNamespace)
		if err != nil {
...
func BackendFactory(l log.Logger, bf proxy.BackendFactory) proxy.BackendFactory {
	return func(remote *config.Backend) proxy.Proxy {
		...
		cfg, err := interpreter.Parse(l, remote.ExtraConfig, BackendNamespace)
		if err != nil {
			if err != interpreter.ErrNoExtraConfig {
				l.Debug(logPrefix, err)
			}
...
func New(cfg interpreter.Config, next proxy.Proxy) proxy.Proxy {
	return func(ctx context.Context, req *proxy.Request) (resp *proxy.Response, err error) {
		...
		for _, source := range cfg.Sources {
			...
			if !ok {
				return nil, interpreter.ErrUnknownSource(source)
			}
			if err := b.DoString(src); err != nil {
				return nil, interpreter.ToError(err)
			}
		}
		...
		if !cfg.SkipNext {
			...
			if err != nil {
				return resp, interpreter.ToError(err)
			}
		} else {
			resp = &proxy.Response{}
		}
		...
		err = interpreter.ToError(b.DoString(cfg.PostCode))
